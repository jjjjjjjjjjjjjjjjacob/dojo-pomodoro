  ...
  return (
    <section className="space-y-3">
      ...
      <div className="rounded border border-foreground/10 p-4 text-sm space-y-2">
        ...
        <div className="flex gap-2">
          <Button disabled={!code || !status || status.status !== "valid"} onClick={async () => { await redeem({ code }); setLastAction("redeemed"); toast.success("Redeemed"); }}>Redeem</Button>
          <Button variant="outline" disabled={!code || !status || status.status !== "redeemed"} onClick={async () => { await unredeem({ code }); setLastAction("unredeemed"); toast.success("Un-redeemed"); }}>Un-redeem</Button>
        </div>
